...
private:
	...
	std::mutex			RoomEntryMutex[MAX_ROOM_COUNT];
	int					RoomEntry[MAX_ROOM_COUNT][MAX_ROOM_USER]{ -1 };
	...
	std::mutex					m_SpaceMutex[MAX_ROOM_COUNT][SPACE_X * SPACE_Y];
	std::unordered_set<int>		m_Space[MAX_ROOM_COUNT][SPACE_X * SPACE_Y];
...
public:
	...
	std::unordered_set<int>& GetSpace(int roomidx, int idx) { return m_Space[roomidx][idx];}
	std::mutex& GetSpaceMutex(int roomidx, int idx) { return m_SpaceMutex[roomidx][idx]; }
	int GetSpaceIndex(int id) {
		CNPC* obj = nullptr;
		//if (id >= NPC_START)		obj = &NPCList[id];
		//else 
		if (id < NPC_START)	obj = &ClientArr[id];
